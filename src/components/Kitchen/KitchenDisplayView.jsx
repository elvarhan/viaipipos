...
import { ChefHat, Clock, Search, CheckCircle2, AlertCircle, Utensils, Filter } from 'lucide-react';

export default function KitchenDisplayView() {
  const { transactions, updateOrderItemStatus, activeBranch, branches } = usePos();
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('ALL'); // 'ALL' | 'PENDING' | 'PROSES' | 'SELESAI'
  const [now, setNow] = useState(Date.now());

  // Timer for real-time queue elapsed calculation
  useEffect(() => {
  ...
  const kitchenOrders = transactions.filter(t => {
    ...
    const kitchenItems = t.items.filter(i => i.station === 'Dapur');
    const isAllDone = kitchenItems.every(i => i.stationStatus === 'Selesai');
    const isAnyWorking = kitchenItems.some(i => i.stationStatus === 'Sedang Dikerjakan');
    ...
    <div style={{ flex: 1, padding: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ backgroundColor: 'rgba(239, 68, 68, 0.15)', color: 'var(--danger)', padding: '8px', borderRadius: 'var(--radius-md)' }}>
              <ChefHat size={24} />
            </div>
            <div>
      ...
      {kitchenOrders.length === 0 ? (
        <div style={{
          ...
          <h3 style={{ fontWeight: 800 }}>Tidak ada pesanan antrean dapur</h3>
          <span style={{ fontSize: '0.85rem' }}>Pesanan makanan baru dari kasir akan muncul secara otomatis di layar ini.</span>
        </div>
      ) : (
        <div style={{
          display: 'grid',
          ...
          {kitchenOrders.map(order => {
            const dItems = order.items.filter(i => i.station === 'Dapur');
            const allItemsDone = dItems.every(i => i.stationStatus === 'Selesai');
            const anyItemWorking = dItems.some(i => i.stationStatus === 'Sedang Dikerjakan');

            return (
              <div key={order.id} style={{
                backgroundColor: 'var(--bg-surface)',
                border: `2px solid ${allItemsDone ? 'var(--success)' : anyItemWorking ? 'var(--warning)' : 'var(--danger)'}`,
                borderRadius: 'var(--radius-lg)',
                overflow: 'hidden',
                display: 'flex',
                flexDirection: 'column',
                ...
                <div style={{
                  padding: '12px 16px',
                  backgroundColor: allItemsDone ? 'rgba(16, 185, 129, 0.15)' : anyItemWorking ? 'rgba(245, 158, 11, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.78rem', fontWeight: 700, display: 'flex', gap: '8px', alignItems: 'center', marginTop: '2px' }}>
                      <span style={{ color: 'var(--accent)' }}>📍 {order.tableName}</span>
                      <span>•</span>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <span style={{
                      ...
                      padding: '2px 8px',
                      borderRadius: '99px',
                      backgroundColor: allItemsDone ? 'var(--success)' : anyItemWorking ? 'var(--warning)' : 'var(--danger)',
                    ...
                    <div key={item.id} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 800, fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <span style={{ backgroundColor: 'var(--primary)', color: '#fff', fontSize: '0.75rem', padding: '1px 6px', borderRadius: '4px' }}>
                            {item.qty}x
                          </span>
                          <span>{item.name}</span>
                        </div>
                        {item.notes && (
                          <div style={{ fontSize: '0.78rem', color: 'var(--danger)', fontWeight: 700, marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
